...
def load_train_data():
    ...
    The function does not take any parameter but relies on the GLOBAL variables from the config
    ...
    for file in tqdm.tqdm(glob.glob(DATA_DIR+FILE_GLOB)):
        try:
...
def load_infer_data(filepath):
    ''' 
    Function to load the inference data from file path and extract the features the same way we did for training.
    '''
